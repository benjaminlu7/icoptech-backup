<?php
/**
 * icoptech ( functions-scripts.php )
 *
 * @package     icoptech
...
add_action(
	...
	function() {
		 ...
		 */
		wp_enqueue_style( 'icoptech-screen', get_theme_file_uri( 'public/css/screen.css' ), array(), '1.0.0' );

		/**
		 * We will be enqueue the app.js file, which mainly be for the navigation only.
		 */
		wp_enqueue_script( 'icoptech-app', get_theme_file_uri( 'public/js/app.js' ), array('jquery'), '1.0.0', true );
...
add_action(
	...
	function() {
		...
		";
		wp_add_inline_style( 'icoptech-style', $custom_css );
	}
);
...
add_action(
	...
	function() {
		wp_enqueue_style( 'icoptech-custom-fonts', get_theme_file_uri( '/vendor/benlumia007/backdrop-core/assets/fonts/custom-fonts.css' ), array(), '1.0.0' );
	}
);